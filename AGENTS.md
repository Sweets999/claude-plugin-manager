...
`cps` (claude-plugin-swapper) is a zero-dependency-at-runtime CLI (one dep: `jsonc-parser`) that switches between named **profiles** of Claude Code plugins and user-defined MCP servers. Plugin on/off state lives in `~/.claude/settings.json` under `enabledPlugins`; user MCP server definitions live in `~/.claude.json` under `mcpServers`. `cps` rewrites those maps (or launches an isolated session) so users can keep their context lean. There is no build step — it's plain ES modules run directly by Node ≥ 18.
...
node bin/cps.js <args>           # run the CLI locally without installing
npm link                         # put `cps` on PATH for manual testing
```
...
- **Errors are typed and carry exit codes** (`src/errors.js`): `0` ok · `2` usage · `3` unknown profile · `4` validation/ambiguity · `5` malformed config/settings. Throw the right `CpsError` subclass rather than calling `process.exit`; `bin/cps.js` catches `isCps` errors and exits with `exitCode`. No new exit codes for MCP (unknown MCP names use the existing StrictValidationError / exit 4).
